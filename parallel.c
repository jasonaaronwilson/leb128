...
// Don't use this code since it is slow. The entire reason to use LEB
// is because smaller numbers are much more common than large ones and
// this optimized for large numbers. The benchmark shows that it is
// probably more than twice as slow as the simpler code.
...
unsigned parallel_encodeULEB128(int64_t Value, uint8_t *p) {
  ...
  // TODO(jawilson): do the small tests first.
  if (Value >= ((1ull << 49) - 1)) {
    n_bytes = 8;
